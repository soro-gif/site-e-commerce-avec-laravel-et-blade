...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.collection.create') }}" class="btn btn-success m-1">
        ...
        <table  id="Collection" class="table">
            ...
            <tbody>
                @foreach($collections as $collection)
						...
						<td>
                    <a href="{{ route('admin.collection.show', ['id' => $collection->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.collection.edit', ['id' => $collection->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $collection->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/collections/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Collection th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Collection');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Collection');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Collection', JSON.stringify(selectedColumns));
        }
