...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.banner.create') }}" class="btn btn-success m-1">
                ...
                @foreach($banners as $banner)
						<tr><td>{{ $banner->id }}</td>
                            ...
                            <div class="form-group d-flex" id="preview_imageUrl" style="max-width: 100%;">
                                <img src="{{  Str::startsWith($banner->imageUrl, 'http') ? $banner->imageUrl : Storage::url($banner->imageUrl) }}"
                                     alt="Prévisualisation de l'image"
                                     style="max-width: 100px; display: block;">
                            </div>
                        </td>
						<td>
                    <a href="{{ route('admin.banner.show', ['id' => $banner->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.banner.edit', ['id' => $banner->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $banner->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/banners/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/banners/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Banner th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Banner');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Banner');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Banner', JSON.stringify(selectedColumns));
        }
