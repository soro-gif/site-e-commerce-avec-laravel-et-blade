...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.social.create') }}" class="btn btn-success m-1">
                ...
                @foreach($socials as $social)
						<tr><td>{{ $social->id }}</td>
                            ...
                            <div class="form-group d-flex" id="preview_imageUrl" style="max-width: 100%;">
                                <img src="{{  Str::startsWith($social->imageUrl, 'http') ? $social->imageUrl : Storage::url($social->imageUrl) }}"
                                     alt="Prévisualisation de l'image"
                                     style="max-width: 100px; display: block;">
                            </div>
                        </td>
						<td>
                    <a href="{{ route('admin.social.show', ['id' => $social->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.social.edit', ['id' => $social->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $social->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/socials/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/socials/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Social th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Social');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Social');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Social', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Social');
